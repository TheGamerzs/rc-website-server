...
const WebsiteCashoutResolvers = {
    Query: {
        getWebUsers: authenticateResolver(
            { tt: [AppConfigs.ttpermissions.ADMIN] },
            (parent, args, { db }, info) => db.website.findAll()
        ),
    },
    Mutation: {
        set_user_perm: (parent, { id, perm }, { db }, info) =>
            db.website
                .update({ permission: perm }, { where: { id: id } })
                .then(() => {
                    return true;
                })
                .catch((err) => {
        ...
        set_user_in_game_id: (parent, { id, in_game_id }, { db }, info) =>
            db.website
                .update({ in_game_id: in_game_id }, { where: { id: id } })
                .then(() => {
                    return true;
                })
                .catch((err) => {
