            ...
            .then(([manager]) => {
                ...
                if (manager.active) {
                    sendStaffNotfication(
                        `@everyone <@${member.discord_id}> is now a manager!`
                    );
                } else {
                    sendStaffNotfication(
                        `<@${member.discord_id}> is no longer a manager :(`
                    );
                }
...
export const setCompany = (db, user, uid, company) => {
    ...
    return db.members.findByPk(uid).then((member) => {
        ...
        sendStaffNotfication(
            `<@${user.id}> has changed **(${member.in_game_id}) ${member.in_game_name}**'s company to **${member.company}**`
        );
...
export const fireMember = (db, user, uid, reason, welcome) => {
    ...
    return db.members.findByPk(uid).then((member) => {
        ...
        sendStaffNotfication(
            `<@${user.id}> has fired **${member.in_game_name}** (**${
                member.in_game_id
            }**). <@${member.discord_id}> with reason: **${
                member.fire_reason
            }**. They are${!member.welcome ? " ***NOT*** " : " "}welcome back.`
        );
...
export const setDeadline = (db, user, uid, deadline) => {
    ...
    return db.members.findByPk(uid).then((member) => {
        ...
        sendStaffNotfication(
            `<@${user.id}> has changed <@${member.discord_id}> (${
                member.in_game_id
            }) **${member.in_game_name}**'s deadline to **${new Date(
                deadline
            ).toDateString()}**`
        );
...
export const setWelcome = (db, user, uid, welcome) => {
    return db.members.findByPk(uid).then((member) => {
        ...
        sendStaffNotfication(
            `<@${user.id}> has marked **${member.in_game_name}** (**${
                member.in_game_id
            }**) as **${welcome ? "welcome" : "unwelcome"}**!`
        );
