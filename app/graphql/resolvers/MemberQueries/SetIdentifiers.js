...
export const changeMemberIdentifiers = (
    db,
    user,
    uid,
    new_name,
    ...
    return db.members.findByPk(uid).then((member) => {
        ...
        sendStaffNotfication(
            `<@${
                user.id
            }> has changed **<@${oldDiscord}> (${oldId}) ${oldName}**'s info. **In Game Name**: ${
                member.in_game_name !== oldName
                    ? `**${member.in_game_name}**`
                    : member.in_game_name
            }. **In Game ID**: ${
                member.in_game_id !== oldId
                    ? `**${member.in_game_id}**`
                    : member.in_game_id
            }. **Discord ID**: ${
                member.discord_id !== oldDiscord
                    ? `**${member.discord_id}**`
                    : member.discord_id
            }`
        );
